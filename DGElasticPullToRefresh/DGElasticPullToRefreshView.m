...
static CGFloat WaveMaxHeight = 90.0;
static CGFloat MinOffsetToPull = 95.0;
static CGFloat LoadingContentInset = 50.0;
static CGFloat LoadingViewSize = 30.0;
...
- (CGFloat)currentWaveHeight{
    return MIN(self.bounds.size.height / 3.0 * 1.6, WaveMaxHeight);
}
...
- (void)scrollViewDidChangeContentOffset:(BOOL)dragging{
    ...
    if (self.state == DGElasticPullToRefreshStateStoped && dragging ) {
        ...
        CGFloat pullProgress = offsetY / MinOffsetToPull;
...
- (void)resetScrollViewContentInset:(BOOL)shouldAddObserverWhenFinished animated:(BOOL)animated completion:(void(^)())completion{
    ...
    if (self.state == DGElasticPullToRefreshStateAnimatingBounce) {
        ...
        contentInset.top += LoadingContentInset;
    }
...
- (void)animateBounce{
    ...
    CGFloat centerY = LoadingContentInset;
    ...
    [UIView animateWithDuration:duration * 0.4 animations:^{
        ...
        self.bounceAnimationHelperView.center = CGPointMake(0, contentInsetTop + LoadingContentInset);
...
- (void)layoutLoadingView{
    ...
    CGFloat loadingViewSize = LoadingViewSize;
    
    CGFloat minOriginY = (LoadingContentInset - loadingViewSize) / 2.0;
